...
int32_t main(){
    ...
    Dinic din(6), nid(6);
    for(int i=0; i<3; ++i){
        din.add_to_src(i+1, arr[i]);
        nid.add_to_src(i+1, arr[i]);
    }
    for(int i=3; i<6; ++i){
        din.add_to_sink(i+1, arr[i]);
        nid.add_to_sink(i+1, arr[i]);
    }
    ...
    for(int i=1; i<=3; ++i)
        nid.add_edge(i, i+3, n);
    nid.add_edge(1, 6, n);
    nid.add_edge(2, 4, n);
    nid.add_edge(3, 5, n);

    cout << n - nid.max_flow() << ' ' << din.max_flow() << endl;
    return 0;
}
